...
import { NextJsImage } from "@/components/template/bites-detail-next";
...
import { GalleriesType } from "../page";



export default function EventPage({galleries}:{galleries:GalleriesType}) {
  const [current, setCurrent] = useState<string | null | undefined>(null)
  const currentItem = galleries?.find((item, index) => (item?.label) === current)
    ...
    <>
      ...
      <div className="transition-all z-30 relative duration-[3s] w-full ease-in-out isolate mx-auto">
        <div className="z-10 relative mx-auto max-w-[65ch] ">
            ...
            <div className=" animate-fade-up w-full animate-delay-[800ms] pointer-events-auto">
              ...
              <img loading="lazy" className="max-w-[min(20vw,80px)] mx-auto mb-24" src="/gif/nhen.webp"></img>
                <ul className="grid grid-cols-3 gap-6">
                  {
                    galleries?.map((field, index) => {
                      let title = field?.label
                      let image = field?.items?.[0]
                      if(!image) 
                        return null
          ...
          <div id={'content'} className="mt-[5vh]">
      ...
      {currentItem && <Gallery title={currentItem?.label + ''} onClose={() => setCurrent(null)} images={((currentItem?.items || []).map(item => item + ''))} />}
