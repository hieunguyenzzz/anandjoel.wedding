...
function Detail({ images, defaultIndex = 0, onClose }: { images: string[], defaultIndex: number, onClose: () => void }) {
  const [photoIndex, setindex] = useState(defaultIndex)
  const [state, setState] = useState()
  if (!images) return null
  return (
    < >
      <style dangerouslySetInnerHTML={{ __html: `html{margin-right:${window.innerWidth - document.body.clientWidth}px;overflow:hidden}` }}>
      </style>
      <Lightbox
