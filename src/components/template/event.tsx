      ...
      <div style={{
        ...
        <Image src={bgImage} priority placeholder='blur' fill className='absolute -z-10 top-0 left-0 w-full' />
        {
          data.fields?.map((item, index) => {
            return <div key={index} data-tina-field={item ? tinaField(item, "en") : undefined} className='prose prose-hr:max-w-[50%] prose-hr:mx-auto  text-[12px] lg:text-[calc(min(2vw,30px))] mx-auto whitespace-pre-wrap prose-h3:font-title prose-p:my-[0.3em] text-center prose-img:m-auto prose-h3:py-1 prose-h3:text-[1.4em] prose-h3:font-bold'><TinaMarkdown content={item?.en} /></div>
          })
        }
